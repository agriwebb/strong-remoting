...
HttpInvocation.isAcceptable = function(val, accept) {
  ...
  if (accept.type) {
    ...
    if (Array.isArray(accept.type)) {
      type = 'array'
    } else if (typeof accept.type === 'string') {
      type = accept.type && accept.type.toLowerCase();
    } else if (typeof accept.type === 'object') {
      type = 'object';
    }
  }
...
HttpInvocation.prototype.createRequest = function() {
  ...
  // disabling fetch because of conflict with Zone in Sept 10th 2019 Webivew update
  req.mode = 'disable-fetch';
